...
from config import DEVICE, TYPE_FORGET, GENRE_TO_FORGET, MAX_EPOCHS
...
def evaluate(model, data_loader, label_encoder):
    ...
    if TYPE_FORGET is None:
        plt.title(f"Confusion Matrix for GENRE CLASSIFICATION, {MAX_EPOCHS} epochs")
    else:
        plt.title(f"Confusion Matrix for LEARNING WITHOUT {GENRE_TO_FORGET}, {MAX_EPOCHS} epochs")

    plt.show()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")

    plt.savefig(f"confusion_matrix/CM_learning_{timestamp}_forget_{TYPE_FORGET}.png", bbox_inches='tight')  # bbox_inches='tight' evita tagli sulle etichette

    return acc

def evaluate_unlearning(model, forget_loader, retain_loader, val_loader, acc_before, label_encoder):
    """
    ...
    disp.plot(ax=ax, cmap=plt.cm.Blues, colorbar=False)
    plt.xticks(rotation=90)
    plt.title(f"Confusion Matrix for UNLEARNING of {GENRE_TO_FORGET}, {MAX_EPOCHS} unl epochs")
    plt.show()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")

    plt.savefig(f"confusion_matrix/CM_unlearning_{timestamp}_forget_{TYPE_FORGET}.png", bbox_inches='tight')  # bbox_inches='tight' evita tagli sulle etichette
